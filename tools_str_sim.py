...
def str_sub(old_str):
    try:
        ...
        if new_str.find(" feat. ") != -1:
            tmp_str = new_str[:new_str.find(" feat. ")]
            new_str = tmp_str.strip() if tmp_str.strip() else new_str
        new_str = latinize(new_str)
        new_str = remove_punct(new_str.strip())
        return new_str
    except Exception as e:
        traceback.print_exc()
    return old_str.lower().strip()
...
def str_sim(str1_old, str2_old):
    ...
    is_same = ratio >= THREADHOLD or format_str1 == format_str2
    if is_same:
        return is_same, "100"
    ...
    ratio = fuzz.ratio(format_str1, format_str2)
    is_same = ratio >= (THREADHOLD + 5) or format_str1 == format_str2
    return is_same, str(ratio)
...
if __name__ == "__main__":
    ...
    print a, latinize(a)
    print c, latinize(c.decode())
    d = "First Of The Year (Equinox)"
    print d, str_sub(d)

    print "*"*30
    print str_sim("BulletProof", "Bullet Proof")
    aa = "Harder, Better, Faster, Stronger"
    print str_sub(aa)

    c1 = "Con Calma (Remix)"
    c2 = "Con Calma feat. Snow"
    print "c1: ", c1
    print "c2: ", c2
    c11 = str_sub(c1)
    ...
    for m2 in m2_list:
        for m1 in m1_list:
            print "M1: ", m1
            print "M2: ", m2
            print "Sim: ", str_sim(m1, m2)
            print
    """
    astr = 'a,b"c'
    print (remove_punct(astr))
